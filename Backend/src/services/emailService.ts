            ...
            <tr>
              <td style="padding:16px 40px 8px;">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                  ...
                  ${totalRow("Delivery", order.shippingPrice > 0 ? inr(order.shippingPrice) : "Free")}
                  ...
                  ${totalRow(isCod ? "Amount due on delivery" : "Total paid", inr(order.totalPrice), true)}
                </table>
              </td>
            </tr>
            <tr>
...
function orderConfirmationText(order: IOrder): string {
  ...
  return [
    ...
    `${isCod ? "Amount due on delivery" : "Total paid"}: ${inr(order.totalPrice)}`,
    ``,
    isCod
